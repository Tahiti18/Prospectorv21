      ...
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          <h1 className="text-4xl font-black uppercase tracking-tighter text-white flex items-center gap-3">
            ARTICLE <span className="text-emerald-500 italic">INTEL</span>
          </h1>
          <p className="text-[11px] text-slate-500 font-bold uppercase tracking-[0.2em]">
            Rapid synthesis for competitive research and repurposing.
          </p>
        </div>
        <div className="flex gap-4">
           {['EXECUTIVE BRIEF', 'VIRAL HOOK PACK', 'STRATEGY AUDIT'].map((t, i) => (
             <button 
              key={i} 
              onClick={() => setMode(t)}
              className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest border-2 transition-all ${mode === t ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white'}`}>
               {t}
             </button>
           ))}
        </div>
        ...
        <div className="lg:col-span-5 space-y-10">
           <div className="bg-[#0b1021] border border-slate-800 rounded-[56px] p-12 shadow-2xl space-y-10">
              <div className="space-y-6">
                 <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">INPUT SOURCE</h3>
                 <textarea 
                   value={source}
              ...
              <button 
                ...
                disabled={isLoading}
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white py-6 rounded-[24px] text-[12px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-4 transition-all active:scale-95 border-b-4 border-emerald-700"
              >
              ...
              <div className="flex-1 p-12 overflow-y-auto custom-scrollbar">
                 {isLoading ? (
                    <div className="h-full flex flex-col items-center justify-center space-y-6">
                       ...
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse italic">Crystallizing Insight...</p>
                    </div>
                 ) : output ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-20 opacity-10 grayscale scale-110">
                       <h4 className="text-4xl font-black italic text-white uppercase tracking-tighter">HUB IDLE</h4>
                    </div>
                 )}
              </div>
           </div>
