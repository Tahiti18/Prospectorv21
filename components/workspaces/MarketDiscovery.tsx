          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              <select 
                ...
                className="w-full bg-[#020617] border-2 border-slate-800 rounded-xl px-5 py-4 text-sm font-bold text-white focus:outline-none focus:border-emerald-500 transition-all shadow-inner appearance-none cursor-pointer"
              >
                {Object.entries(STRATEGIC_NICHES).map(([group, items]) => (
                  <optgroup key={group} label={group} className="bg-[#0b1021] text-emerald-500 font-black text-[10px] uppercase">
                    {items.map(item => (
                      <option key={item} value={item} className="bg-[#020617] text-white text-sm font-bold">
                        {item.toUpperCase()}
                      </option>
                    ))}
                  </optgroup>
