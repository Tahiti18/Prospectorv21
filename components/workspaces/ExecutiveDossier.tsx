    ...
    <div className="bg-white text-slate-900 min-h-screen font-sans selection:bg-emerald-100 dossier-container">
      ...
      <style>{`
        @media print {
          ...
          .dossier-page { height: 297mm; page-break-after: always; padding: 25mm !important; overflow: hidden; display: flex; flex-col; justify-content: center; }
      ...
      <section className="dossier-page flex flex-col justify-center items-center text-center bg-[#020617] text-white">
        <div className="absolute top-12 left-12 flex items-center gap-3">
          ...
          <span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-40">PROSPECTOR_OS // STRATEGIC_ASSET</span>
        </div>
        
        <div className="space-y-12">
            <h4 className="text-[10px] font-black uppercase tracking-[0.6em] text-emerald-500">EXECUTIVE TRANSFORMATION BLUEPRINT</h4>
      ...
      {/* PAGE 4: VISUAL DIRECTIVES */}
      <section className="dossier-page">
        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] mb-12 border-b-2 border-slate-900 pb-4 w-fit">03 // AESTHETIC INDOCTRINATION</h2>
        <div className="space-y-12">
                ...
                <div className="flex gap-4 mt-12">
                    {data.visualDirection?.colorPalette?.map((c: any, i: number) => (
                        <div key={i} className="flex flex-col items-center gap-2">
                            <div className="w-16 h-16 rounded-2xl border-2 border-white/10" style={{ backgroundColor: c.hex }}></div>
                            <span className="text-[8px] font-bold opacity-50">{c.hex}</span>
                        </div>
                    ))}
                </div>
            </div>
      ...
      {/* PAGE 5: OUTREACH & SEQUENCE */}
      <section className="dossier-page">
        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] mb-12 border-b-2 border-slate-900 pb-4 w-fit">04 // DEPLOYMENT PROTOCOL</h2>
           ...
           {data.outreach?.emailSequence?.slice(0, 3).map((email: any, i: number) => (
             <div key={i} className="p-8 border border-slate-200 rounded-3xl space-y-4">
                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">DAY {email.day || i+1} // OUTBOUND</span>
                    <span className="text-[10px] font-black text-emerald-600 uppercase italic">{email.subject}</span>
                </div>
                <p className="text-xs text-slate-500 font-medium leading-relaxed italic whitespace-pre-wrap">{email.body}</p>
             </div>
           ))}
           <div className="p-8 bg-slate-900 text-white rounded-[32px] flex items-center gap-8">
                <div className="w-16 h-16 bg-emerald-600 rounded-2xl flex items-center justify-center text-3xl">📞</div>
                <div className="flex-1">
                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest block mb-1">CALL_PROTOCOL</span>
                    <p className="text-sm italic font-medium">"{data.outreach?.callScript?.hook}"</p>
                </div>
           </div>
        </div>
      </section>

      {/* PAGE 6: FUNNEL & ROADMAP */}
      <section className="dossier-page">
        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] mb-12 border-b-2 border-slate-900 pb-4 w-fit">05 // CONVERSION GEOMETRY</h2>
        <div className="space-y-8 max-w-2xl mx-auto">
            {data.funnel?.map((step: any, i: number) => (
                <div key={i} className="flex items-center gap-12 relative">
                    ...
                    <div className="w-14 h-14 rounded-full border-2 border-slate-900 flex items-center justify-center font-black italic text-xl shrink-0">0{i+1}</div>
                    <div className="flex-1">
                        <h4 className="text-lg font-black uppercase italic text-slate-900">{step.title}</h4>
                        <p className="text-xs text-slate-500 font-medium italic mt-1 leading-relaxed">"{step.description}"</p>
                        <div className="mt-2 flex gap-4">
                            <span className="text-[8px] font-black bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded uppercase">{step.conversionGoal}</span>
                            <span className="text-[8px] font-black bg-slate-100 text-slate-500 px-2 py-0.5 rounded uppercase">AI_ENHANCED</span>
                        </div>
                    </div>
                </div>
            ))}
        ...
        <div className="mt-auto pt-12 border-t border-slate-100 text-center">
             ...
             <p className="text-[8px] font-mono text-slate-300 mt-4 uppercase">VERIFIED BY PROSPECTOR OS V3.2</p>
        </div>
      </section>
    </div>
  );
