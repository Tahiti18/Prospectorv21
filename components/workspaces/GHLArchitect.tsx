...
export const GHLArchitect: React.FC<GHLArchitectProps> = ({ lead, leads, onLockLead }) => {
  const [steps, setSteps] = useState<BoardroomStep[]>([
    { agentName: 'STRATEGIST', role: 'Architectural Analyst', modelLabel: 'Gemini 3.0 Flash', modelId: 'google/gemini-3-flash-preview', status: 'WAITING', currentRound: 1 },
    { agentName: 'COMPLIANCE', role: 'Risk & Audit Control', modelLabel: 'Llama 3.1 70B', modelId: 'meta-llama/llama-3.1-70b-instruct', status: 'WAITING', currentRound: 1 },
    { agentName: 'ENGINEER', role: 'Logic Specialist', modelLabel: 'Mistral Large 2', modelId: 'mistralai/mistral-large', status: 'WAITING', currentRound: 1 },
    { agentName: 'EXECUTIVE', role: 'Synthesis Lead', modelLabel: 'Gemini 3.0 Flash', modelId: 'google/gemini-3-flash-preview', status: 'WAITING', currentRound: 1 }
  ]);
  ...
  const handleExecuteBuild = async () => {
    ...
    try {
       await ghlAutoBuilder.executeBuild(techBlueprint, (msg) => {
         setBuildLogs(prev => [...prev, `[PROCESS] ${msg}`]);
       });
  ...
  const handleMockAuth = async () => {
     await ghlAutoBuilder.authorizeLocation('mock_auth_node');
     setIsAuth(true);
     toast.success("Deployment Node Authorized.");
  };
    ...
    <div className="max-w-[1700px] mx-auto py-12 space-y-16 animate-in fade-in duration-700 pb-96 px-6">
      <div className="flex flex-col md:flex-row justify-between items-end border-b-2 border-emerald-500/20 pb-12 gap-8">
        <div className="space-y-3">
          <h1 className="text-4xl font-black italic text-white uppercase tracking-tighter leading-none">
            SOLUTIONS <span className="text-emerald-500 not-italic">ARCHITECT</span>
          </h1>
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.6em] italic">System Design Engine v1.1 // {lead ? `Client: ${lead.businessName}` : 'AWAITING SELECTION'}</p>
        </div>
        
        <div className="flex items-center gap-6 bg-[#0b1021] p-6 rounded-[32px] border border-slate-800 shadow-2xl">
           {!isAuth && (
             <button onClick={handleMockAuth} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">LINK DEPLOYMENT NODE</button>
           )}
           <div className="flex flex-col items-end">
              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-2">TARGET SELECTION</span>
              <select 
                value={lead?.id || ''}
           ...
           {!isExecuting && !uiContent ? (
             ...
             >
               {lead ? 'CONVENE ANALYSTS' : 'TARGET REQUIRED'}
             </button>
           ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {steps.slice(0, 2).map((step, i) => (
          <div key={i} ref={cardRefs[i]}><AgentNode step={step} /></div>
        ))}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {steps.slice(2, 4).map((step, i) => (
          <div key={i+2} ref={cardRefs[i+2]}><AgentNode step={step} /></div>
        ))}
      </div>
        ...
        <div ref={finalRef} className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in slide-in-from-bottom-20 duration-1000">
           <div className="lg:col-span-8 bg-black border-4 border-emerald-500/50 rounded-[64px] shadow-2xl p-20 relative overflow-hidden">
              <div className="mb-16 border-b border-slate-800 pb-12">
                <h2 className="text-4xl font-black italic text-white uppercase tracking-tighter leading-none mb-4">MASTER <span className="text-emerald-500">SCHEMATIC</span></h2>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.6em]">VISUAL SYSTEM PREVIEW // {techBlueprint?.meta?.target_business}</p>
              </div>
           ...
           <div className="lg:col-span-4 flex flex-col gap-8">
              <div className="bg-[#0b1021] border border-slate-800 rounded-[48px] p-10 shadow-2xl space-y-8 flex flex-col">
                 <div className="space-y-2">
                    <h3 className="text-lg font-black italic text-white uppercase tracking-widest">DEPLOYMENT <span className="text-emerald-500">ENGINE</span></h3>
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.4em]">STATUS: ARCHITECTURE READY</p>
                 </div>
                 ...
                 >
                    {isBuilding ? 'DEPLOYING...' : 'INITIATE AUTO-DEPLOY'}
                 </button>
                 ...
                 <div className="flex-1 bg-black border border-slate-800 rounded-3xl p-6 font-mono text-[10px] h-[500px] overflow-y-auto custom-scrollbar space-y-2 shadow-inner">
                    {buildLogs.length === 0 ? (
                       <div className="h-full flex items-center justify-center text-slate-800 italic uppercase">Awaiting process initiation...</div>
                    ) : (
                       buildLogs.map((log, i) => (
                          <div key={i} className={`pb-2 border-b border-white/5 last:border-0 ${log.includes('SUCCESS') ? 'text-emerald-400' : 'text-slate-500'}`}>
                             {log}
                          </div>
                       ))
                    )}
    ...
    return (
        <div className={`bg-[#0b1021] border-4 rounded-[64px] p-12 transition-all duration-1000 relative overflow-hidden flex flex-col ${
            ...
            <div className="flex justify-between items-start mb-12 relative z-10">
               <div className="space-y-2">
                  <h3 className="text-2xl font-black italic text-white uppercase tracking-tighter">{step.agentName}</h3>
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em] italic">{step.role}</p>
               </div>
               <div className={`px-6 py-2 rounded-xl border-2 text-[10px] font-black uppercase tracking-[0.2em] ${isActive ? 'bg-emerald-500 text-black border-emerald-400 animate-pulse' : 'bg-slate-950 text-slate-600 border-slate-800'}`}>{step.status}</div>
            </div>
            <div className="flex-1 flex flex-col min-h-0 relative z-10">
                {isActive && <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6"><div className="w-16 h-16 border-4 border-emerald-900 border-t-emerald-500 rounded-full animate-spin"></div><p className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.6em] animate-pulse">ACTIVE ANALYSIS...</p></div>}
                {step.output && (
                    <div className="h-full overflow-y-auto custom-scrollbar pr-4 space-y-6">
                        {step.output.split('\n').filter(l => l.trim() !== '').map((line, idx) => (
                           <p key={idx} className="text-[13px] text-slate-400 leading-relaxed font-sans border-l-2 border-emerald-500/10 pl-6 italic">{line.trim()}</p>
                        ))}
                    </div>
                )}
            </div>
